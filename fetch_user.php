        ...
        <table class="table table-bordered table-striped">
            ...
            foreach($result as $row){
                ...
                if($user_last_activity > $current_timestamp)
                {
                    ...
                    $status='<span class="label label-danger">Deconnecté</span>';
                }
                $output .='<tr>
                    <td>'.$row['username'].' '.count_unseen_message($row['user_id'],$_SESSION['user_id'],$connect).' '.fetch_is_type_status($row['user_id'],$connect).'</td>
                    <td>'.$status.'</td>
                    <td><button type"button" class="btn btn-info btn-xs start-chat" data-touserid="'.$row['user_id'].'" data-tousername="'.$row['username'].'">Start Chat</button></td>
                </tr>
                ';
            }
        $output .='</table>';
