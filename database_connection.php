<?php
    ...
    function fetch_user_chat_history($from_user_id, $to_user_id, $connect){
        ...
        $output .='</ul>';
        $query="UPDATE chat_message SET status='0' WHERE from_user_id='".$to_user_id."'
    ...
    function count_unseen_message($from_user_id, $to_user_id, $connect){
        ...
        $output ='';
        if($count > 0){
            $output = '<span class="label label-success" style="background-color:green;padding:10px 10px;border-radius:150px;">'.$count.'</span>';
        }   
        return $output;
    }
    function fetch_is_type_status($user_id,$connect){
        $query="SELECT is_type FROM login_details
        WHERE user_id='".$user_id."' ORDER BY last_activity DESC LIMIT 1";
        $statement=$connect->prepare($query);
        $statement->execute();
        $result=$statement->fetchAll();
        ...
        foreach($result as $row)
        {
            if($row["is_type"]=='yes')
            {
                $output=' - <small><em><span class="text-muted">Entrain d\'ecrire</span></em></small>';
            }
        }
        return $output;
    }
